...
static arr_t(mod_t*) mod_tab = NULL;

void mods_init(void) {
	mod_tab = arr_alloc(mod_t*);
}

void mods_terminate(void) {
	for (usz i = 0; i < arr_count(mod_tab); ++i) {
		mod_t* mod = mod_tab[i];
...
lt_err_t mod_register(mod_t* mod) {
	...
	usz idx = at - mod_tab;
	arr_insert(mod_tab, idx, mod);
	return LT_SUCCESS;
}

mod_t* mod_find(lstr_t name) {
	mod_t** pptr = lookup_mod(arr_count(mod_tab), mod_tab, name);
	if (pptr == NULL)
		return NULL;
	return *pptr;
}
